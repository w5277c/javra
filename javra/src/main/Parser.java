...
public class Parser {
	...
	public Parser(ProgInfo l_pi, String l_filename, File l_file, String l_charset) throws Exception {
		...
		Scanner scanner = new Scanner(l_file, l_charset);
		...
		if(scanner.hasNextLine()) {
			while (scanner.hasNextLine()) {
				...
				if(null == line) {
					...
					JAObject jaobj = line_parse(l_pi, line, l_charset);
					if(null != jaobj) {
						line_cntr++;
			...
			if(0 == l_file.length()) {
				l_pi.print(EMsgType.MSG_WARNING, null, l_filename + ": empty file");
			}
			else {
				l_pi.print(EMsgType.MSG_ERROR, null, l_filename + ": wrong charset encoding(expected " + l_charset + ")");
			}
		}
		scanner.close();
	...
	public static JAObject line_parse(ProgInfo l_pi, Line l_line, String l_charset) {
		...
		if(null != cur_macro && !l_line.get_text().equalsIgnoreCase(".endmacro") && !l_line.get_text().equalsIgnoreCase(".endm")) {
			...
			try {
				return JAObject.parse(l_pi, l_line, l_charset);
			}
			catch(Exception ex) {
